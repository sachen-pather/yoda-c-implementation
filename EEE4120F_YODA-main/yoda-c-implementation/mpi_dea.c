...
int main(int argc, char *argv[]) {
    ...
    if (rank == 0) {
       ...
       (double)load_cycles / total_cycles * 100.0);
        ...
        for (j = 0; j < num_iterations; j++) {
            ...
            if (j == num_iterations - 1) {
                // Decrypt for verification with timing
                dea_reset(&dea);
                ...
                start_cycles = get_cycles();
                if (write_file(encrypted_file, full_encrypted, file_size)) {
       ...
       cycles_to_ms(total_cycles));
        ...
        printf("\nThroughput:\n");
        printf("Encryption: %.2f MB/second\n", encrypt_mb_per_second);
        printf("Decryption: %.2f MB/second\n", decrypt_mb_per_second);
    ...
    // Worker processes
        ...
        for (i = 0; i < rank; i++) {
            preceding_bytes += (file_size / size) + (i < (file_size % size) ? 1 : 0);
        }
